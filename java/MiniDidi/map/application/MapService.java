...
public class MapService {
    ...
    public void validateFilePaths() {
        ...
        try {
            if (!mapFileResource.exists()) {
                logger.warn("map file does not exist：{}", mapFileResource.getFile().getPath());
            }
        } catch (IOException e) {
            logger.error("map file does not exist：{}", e.getMessage());
        }
    }
     ...
     *
     * @return 地图对象
     * @throws IllegalStateException 如果地图对象不存在
     */
    public boolean[][] getMap() {
        boolean[][] map = didiMapDao.getMap();
        if (null == map) {
            throw new IllegalStateException("原始地图尚未初始化或为空");
        } else {
            return map;
        }
    }
     ...
     *
     * @return 反转后的地图二维布尔数组
     * @throws IllegalStateException 如果地图不存在
     */
    public boolean[][] getReverseMap() {
        boolean[][] reverseMap = didiMapDao.getReverseMap();
        if (reverseMap == null || reverseMap.length == 0) {
            // 抛出与 getMap() 一致的异常，确保Service层行为统一
            throw new IllegalStateException("反转地图尚未初始化或为空。可能是原始地图还未加载。");
        }
        return reverseMap;
    }
